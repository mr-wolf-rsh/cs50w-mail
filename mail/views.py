...
def get_recipients(request):
    if request.method == "GET":
        ...
        # )
        emails_sent = request.user.emails_sent.all()
        all_recipients = set(list(chain.from_iterable([email.recipients.all() for email in emails_sent])))
        return JsonResponse([user.serialize() for user in all_recipients], safe=False)
    else:
        return JsonResponse({
            "error": "GET request required."
        }, status=400)
